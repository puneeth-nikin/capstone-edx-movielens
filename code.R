...
edx%>%group_by(year_released)%>%summarise(avg_ratings=mean(rating))%>%ggplot(aes(year_released,avg_ratings))+geom_point()+geom_smooth(method = "loess")+theme(axis.text.x = element_text(angle = 90,hjust = 1))+ggtitle("Average ratings vs Year Released")
edx%>%group_by(year_released)%>%summarise(avg_ratings=mean(rating))%>%cor()
edx%>%group_by(year_released)%>%summarise(n_ratings=n())%>%ggplot(aes(year_released,n_ratings))+geom_point()+geom_smooth(method = "loess")+theme(axis.text.x = element_text(angle = 90,hjust = 1))+ggtitle("Number of Ratings vs Year Released ")

#exploring the relationship between ratings and when it was rated
edx%>%mutate(week_rated=round_date(datetime,unit = 'week'))%>%group_by(week_rated)%>%summarise(avg_rating=mean(rating))%>%ggplot(aes(week_rated,avg_rating))+geom_point()+geom_smooth(method="loess")+ggtitle("Average Rating vs Timestamp of Rating")
edx%>%mutate(week_rated=round_date(datetime,unit = 'week'))%>%group_by(week_rated)%>%summarise(n_rating=n())%>%ggplot(aes(week_rated,n_rating))+geom_point()+geom_smooth(method="loess")+ggtitle("Number of Ratings vs Timestamp of Rating")
...
results<-rbind(results,data.frame(model='Movie Effect',result=result_movie_effect))
...
results<-rbind(results,data.frame(model='Movie+User Effect',result=result_movie_user_effect))
...
results<-rbind(results,data.frame(model='Movie+User+genre Effect',result=result_movie_user_genre_effect))
...
result_movie_user_genre_year_effect<-RMSE(predictions_movie_user_genre_year_effect,test_set$rating)
results<-rbind(results,data.frame(model='Movie+User+genre+year_released Effect',result=result_movie_user_genre_year_effect))
...
mu<-mean(edx$rating)
avg_ratings_movie<-edx%>%group_by(movieId)%>%summarise(b_i=sum(rating-mu)/(n()+best_lambda))
avg_ratings_user<-edx%>%left_join(avg_ratings_movie,by='movieId')%>%group_by(userId)%>%summarise(b_u= sum(rating-mu-b_i)/(n()+best_lambda))
avg_ratings_genre<-edx %>%left_join(avg_ratings_movie,by='movieId')%>%left_join(avg_ratings_user,by='userId')%>%group_by(genres)%>%summarise(b_g=sum(rating-mu-b_i-b_u)/(n()+best_lambda))
avg_ratings_year<-edx%>%left_join(avg_ratings_movie,by='movieId')%>%left_join(avg_ratings_user,by='userId')%>%left_join(avg_ratings_genre,by='genres')%>%group_by(year_released)%>%summarise(b_t=sum(rating-mu-b_i-b_u-b_g)/(n()+best_lambda))

#predictions on the validation set
validation<-validation%>%mutate(year_released=year(as.Date(str_match(title,regex_pattern)[,2],format="%Y")))
predictions<-validation%>%left_join(avg_ratings_movie,by='movieId')%>%left_join(avg_ratings_user,by='userId')%>%left_join(avg_ratings_genre,by='genres')%>%left_join(avg_ratings_year,by='year_released')%>%mutate(pred=mu+b_i+b_u+b_g+b_t)%>%pull(pred)
